...
use kokoro_default_impl::plugin::{Result, ScopeIdGen};
pub use libloading;
...
pub trait DynamicPluginable<T: Resource> {
    ...
    fn plugin_dynamic(&self, lib: Arc<Library>, config: Option<Value>) -> Result<ScopeId>;
}
...
impl<R: Resource + 'static, M: Mode + 'static> DynamicPluginable<R> for Context<R, M> {
    ...
    fn plugin_dynamic(&self, lib: Arc<Library>, config: Option<Value>) -> Result<ScopeId> {
        ...
        scope.cache().insert("kokoro-dynamic-plugin/lib-cache", lib);
